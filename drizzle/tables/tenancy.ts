// drizzle/tables/tenancy.ts
//
// Tenancy baseline (post-domain-prune, org-based tenancy):
...
//                       discriminator and matches a list_of_values row of
//                       type='TENANT_VALUES'.
//   4. audit_logs     — generic field-level audit trail (tenant-scoped)
//
// Tenancy (project_conventions §6): `tenant_id === Clerk org_id`. A tenant is
// a Clerk organization, not a local row — there is no `tenants` table and no
// `memberships` table.
