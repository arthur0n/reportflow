// api/render/money.ts
//
// PORTED VERBATIM (modulo import extensions) from poc/lib/money.ts — proven
// code, decisions §11's provenance rule applies: copy from what ran, do not
// re-derive.
//
...
//
// Why this matters for the product's core claim (HANDOFF "deterministic where
// it can be"): a number rendered in the report is byte-identical to the number
 ...
 *
 * Both renderings occur in the corpus this was proven against: the invoices
 * use a dot separator and the € sign, the contract spells the currency out and
 * separates thousands with a space. Same amount, two typographies — so the
 * parse tolerates both and the ARITHMETIC is identical either way. Throws on
 * anything it cannot read exactly; a silent zero here would be a wrong number
...
export function parseEuroToCents(verbatim: string): number {
  ...
  if (match === null) {
    throw new Error(`não é moeda europeia reconhecível: ${JSON.stringify(verbatim)}`);
  }
...
/** Whether a string is money this module can parse, without throwing.
 *
 * The aggregate builder (api/render/report-context.ts) has to decide whether a
 * role's field list carries the conventional money totals BEFORE it commits to
 * summing them — a probe that throws would make "this role has no totals" an
 * exception path rather than an ordinary answer. */
export function isParsableMoney(value: unknown): value is string {
...
export function dateKey(ddmmyyyy: string): number {
  ...
  if (m === null) {
    throw new Error(`data inválida: ${ddmmyyyy}`);
  }
  const [, d = "", mo = "", y = ""] = m;
...
export function monthLabel(ddmmyyyy: string): string {
  ...
  if (name === undefined) {
    throw new Error(`mês inválido: ${mo}`);
  }
  return `${name} de ${y}`;
}
