...
1. **Parse the request precisely.** Identify the exact page/component, the exact element, and the exact change. If the user names a page (e.g. "dashboard page"), look under `app/src/pages/` for the matching route/component. If ambiguous, ask one targeted question — do not guess across multiple files.
